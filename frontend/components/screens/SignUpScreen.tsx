...
export const SignUpScreen: React.FC = () => {
  const { registerUser, navigate, showToast } = useSpotFree();
  ...
  const handleProceedToOtp = (e: React.FormEvent) => {
    ...
    setSignupStep('otp');
    showToast(`OTP sent to ${email} (Demo code: ${code})`, 'mark_email_read');
  };
  ...
  const handleResendOtp = () => {
    ...
    setOtpError('');
    setResendCountdown(30);
    showToast(`New verification code sent! (Demo code: ${code})`, 'mark_email_read');
  };
  ...
  const handleVerifyAndCreate = (e: React.FormEvent) => {
    ...
    if (otpValue.trim() !== generatedOtp && otpValue.trim() !== '123456') {
      setOtpError('Invalid verification code. Please check the code sent to your email.');
      showToast('Invalid verification OTP', 'error');
      return;
    }
    ...
    const res = registerUser({
      ...
      branch: selectedRole === 'Student' ? branch.trim() : undefined,
      year: selectedRole === 'Student' ? year.trim() : undefined,
      semester: selectedRole === 'Student' ? semester.trim() : undefined,
      group: selectedRole === 'Student' ? group.trim() : undefined,
    });
    ...
    <div className="flex flex-col w-full px-5 py-6 min-h-screen bg-[#f8f9ff]">
      ...
      {signupStep === 'success' ? (
        ...
        <form onSubmit={handleVerifyAndCreate} className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200 flex flex-col gap-4">
          ...
          <div className="p-2.5 rounded-xl bg-emerald-50 border border-emerald-200 flex items-center justify-between">
            ...
            <button
              ...
              className="text-[10px] font-bold text-emerald-700 bg-emerald-100 hover:bg-emerald-200 px-2 py-1 rounded cursor-pointer"
            >
              Auto-fill
            </button>
          </div>
        ...
        <form onSubmit={handleProceedToOtp} className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200 flex flex-col gap-3.5">
          ...
          {/* Password */}
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-slate-700" htmlFor="signup-pwd">
              Password
            </label>
            <div className="relative flex items-center">
              <span className="material-symbols-outlined absolute left-3 text-slate-400 text-lg pointer-events-none">
              ...
              <input
                ...
                className={`w-full h-11 pl-9 pr-10 rounded-lg bg-slate-50 border text-xs text-slate-900 focus:bg-white focus:outline-none transition-colors ${
                  errors.password ? 'border-rose-400 focus:border-rose-500' : 'border-slate-200 focus:border-slate-400'
                }`}
              />
              <button
                type="button"
            ...
            {errors.password && (
              <span className="text-[11px] text-rose-600 font-medium">{errors.password}</span>
            )}
          </div>

          {/* Confirm Password */}
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-slate-700" htmlFor="signup-confirm-pwd">
              Confirm Password
            </label>
            <div className="relative flex items-center">
              <span className="material-symbols-outlined absolute left-3 text-slate-400 text-lg pointer-events-none">
              ...
              <input
                ...
                className={`w-full h-11 pl-9 pr-10 rounded-lg bg-slate-50 border text-xs text-slate-900 focus:bg-white focus:outline-none transition-colors ${
                  errors.confirmPassword ? 'border-rose-400 focus:border-rose-500' : 'border-slate-200 focus:border-slate-400'
                }`}
              />
              <button
                type="button"
            ...
            {errors.confirmPassword && (
              <span className="text-[11px] text-rose-600 font-medium">{errors.confirmPassword}</span>
            )}
          </div>
          ...
          <button
            type="submit"
            className="w-full h-12 rounded-xl bg-[#0f172a] hover:bg-slate-800 active:scale-[0.98] text-white font-bold text-xs flex items-center justify-center gap-2 shadow-md transition-all mt-2 cursor-pointer"
          >
