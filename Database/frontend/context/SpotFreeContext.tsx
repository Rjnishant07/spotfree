...
interface SpotFreeContextType {
  ...
  loginWithProfile: (user: UserProfile, opts?: { silent?: boolean }) => void;
  authChecked: boolean;
...
/**
 * Live view of the stored rooms: timetable engine + reservation expiry (pure, no history side effects).
 * The server runs the same rules and persists expiries; this keeps the UI ticking between polls.
 */
export function deriveRooms(
  prevRooms: Room[],
  now: Date,
  timetableEntries: TimetableEntry[]
): { rooms: Room[]; changed: boolean } {
  const todayStr = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`;
  const nowMinutes = now.getHours() * 60 + now.getMinutes();
  let changed = false;

  const reservationEnded = (r: Room): boolean => {
    if (!(r.status === 'RESERVED' && r.reservedUntil)) return false;
    if ((r.reservationDate || todayStr) !== todayStr) return false;
    const end = parseMinutes(r.reservedUntil);
    return end !== null && nowMinutes >= end;
  };
  const released = (r: Room): Room => ({
    ...r,
  ...
  const rooms = prevRooms.map((r) => {
    if (isProtectedQrRoom(r.id)) {
      if (reservationEnded(r)) {
        changed = true;
        return released(r);
      }
      return r;
    ...
    if (r.isTimetableControlled) {
      if (r.status === 'RESERVED' && r.reservedUntil && (r.statusAuthority === 'ADMIN' || r.statusAuthority === 'FACULTY')) {
        if ((r.reservationDate || todayStr) === todayStr) {
          if (reservationEnded(r)) {
            changed = true;
            return computeTimetableRoomState(r, now, timetableEntries);
          }
          return r;
        }
      }
      const live = computeTimetableRoomState(r, now, timetableEntries);
      if (
        r.status !== live.status ||
        r.timeText !== live.timeText ||
        r.statusAuthority !== live.statusAuthority ||
        JSON.stringify(r.currentClass) !== JSON.stringify(live.currentClass) ||
        JSON.stringify(r.upcomingClass) !== JSON.stringify(live.upcomingClass)
      ) {
        changed = true;
        return live;
      }
...
export function SpotFreeProvider({ children }: { children: React.ReactNode }) {
  ...
  // ---- Server sync: rooms, history and notifications live in the database ----
  const [sessionActive, setSessionActive] = useState(false);
  const pendingWrites = useRef(0);

  const refreshState = useCallback(async (force = false) => {
    if (!force && pendingWrites.current > 0) return; // don't clobber an in-flight optimistic update
    try {
      const res = await fetch('/api/state');
      if (!res.ok) return;
      const d = await res.json();
      const { rooms: live } = deriveRooms(d.rooms as Room[], new Date(), HIT_TIMETABLE);
      setRooms(live);
      setHistory(syncHistoryWithRooms(live, d.history as StatusHistoryItem[]));
  ...
  }, [sessionActive, refreshState]);

  // Send a change to the server; on failure show why and fall back to server truth.
  const syncToServer = useCallback(async (path: string, method: 'POST' | 'PATCH', body: unknown) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        showToast(d.error || 'Could not save your change', 'error');
      }
    } catch {
  ...
  const loginWithProfile = useCallback((user: UserProfile, opts?: { silent?: boolean }) => {
    setCurrentUser(user);
    setSessionActive(true);
    setNavHistory([]);
    const r = user.role.toLowerCase();
    if (r === 'student') setCurrentView('student-dashboard');
    else if (r === 'faculty') setCurrentView('faculty-dashboard');
    else setCurrentView('admin-dashboard');
    if (!opts?.silent) showToast(`Welcome back, ${user.name}!`);
  }, [showToast]);
  ...
  useEffect(() => {
    ...
    fetch('/api/auth/me')
      ...
      .then((d) => {
        if (!cancelled && d?.user) loginWithProfile(d.user, { silent: true });
      })
      .catch(() => {})
      .finally(() => {
  ...
  const registerUser = useCallback(async (newUser: {
    ...
    try {
      ...
      showToast(`Account created for ${data.user.name}!`, 'check_circle');
  ...
  const updateUserProfile = useCallback((updates: Partial<UserProfile>) => {
    setCurrentUser(prev => {
      // Email and role can never be changed from the client
    ...
    const { name, rollNumber, branch, year, semester, group } = updates;
    fetch('/api/auth/profile', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name, rollNumber, branch, year, semester, group }),
    })
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => {
  ...
  const updateRoomStatus = useCallback((
    ...
    syncToServer(`/api/rooms/${encodeURIComponent(targetRoom.id)}/status`, 'POST', {
      newStatus, note, source, reservedUntil, startTime, endTime, date,
    });
    return { success: true };
  }, [rooms, currentUser, currentRole, showToast, syncToServer]);
  ...
  const bookVacantRoom = useCallback((
    ...
    syncToServer(`/api/rooms/${encodeURIComponent(target.id)}/book`, 'POST', {
      date, startTime: startDisplay, endTime: endDisplay,
    });
    return true;
  }, [rooms, currentUser, currentRole, showToast, formatTimeTo12Hr, parseTimeToMinutes, checkOverlap, syncToServer]);
  ...
  const adminOverrideStatus = useCallback((
    ...
    syncToServer(`/api/rooms/${encodeURIComponent(target.id)}/admin-override`, 'POST', {
      newStatus, reservedUntil, note, startTime, endTime, date,
    });
  }, [rooms, currentUser, showToast, syncToServer]);
  ...
  // Keep the UI live between server polls: timetable engine + reservation expiry on real wall-clock time.
  // (The server persists expiries and writes the audit history; this only updates what's displayed.)
  useEffect(() => {
    setRooms((prev) => {
      const { rooms: next, changed } = deriveRooms(prev, currentTime, timetable);
  ...
  const addRoom = useCallback((newRoom: {
    ...
    syncToServer('/api/rooms', 'POST', {
      id: newRoom.id, building: newRoom.building, floor: newRoom.floor, type: newRoom.type, capacity: newRoom.capacity,
    });
  }, [currentUser.name, showToast, syncToServer]);
  ...
  const editRoom = useCallback((
    ...
    if (updates.capacity !== undefined || updates.type !== undefined) {
      syncToServer(`/api/rooms/${encodeURIComponent(roomId)}`, 'PATCH', {
        capacity: updates.capacity, type: updates.type,
      });
    }
  }, [rooms, showToast, updateRoomStatus, syncToServer]);
